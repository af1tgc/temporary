...
CREATE_FILE=`hostname`"_AIX_"`date +%m%d`.txt
...
echo "###################################   AIX Security Check        ######################################"
echo "###################################   AIX Security Check        ######################################" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
echo "############################################ Start Time ################################################"
date
echo " "
...
if [ $vul_state -eq 1 ]; then
	...
	echo "● 1.01 결과 : 수동" >> $CREATE_FILE 2>&1
fi
...
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
if [ `cat /etc/passwd | egrep "^lp|^uucp|^nuucp|^daemon|^bin|^lpd|^nobody|^nobody4|^gopher|^smmsp|^listen|^webservd" | wc -l` -eq 0 ]
  then
    echo "불필요한 계정이 존재하지 않습니다."  >> $CREATE_FILE 2>&1
  else
    cat /etc/passwd | egrep "^lp|^uucp|^nuucp|^daemon|^bin|^lpd|^nobody|^nobody4|^gopher|^smmsp|^listen|^webservd" >> $CREATE_FILE 2>&1
fi

if [ `cat /etc/passwd | egrep -i "^lp|^uucp|^nuucp|^daemon|^bin|^lpd|^nobody|^nobody4|^gopher|^smmsp|^listen|^webservd" | grep -v "lpd" | wc -l` -gt 0 ]
    then
...
if [ -f /etc/security/user ]
  then
   awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minlen" | grep "=" >> $CREATE_FILE 2>&1
  else
     echo "/etc/security/user 파일이 없습니다." >> $CREATE_FILE 2>&1
fi
...
echo " " >> $CREATE_FILE 2>&1
  ...
  if [ -f /etc/security/user ]
    then
     awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "maxage" | grep "=" >> $CREATE_FILE 2>&1
    else
      echo "/etc/security/user 파일이 없습니다." >> $CREATE_FILE 2>&1
  fi
...
if [ -f /etc/security/user ]
    then
     awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minage" | grep "=" >> $CREATE_FILE 2>&1
    else
      echo "/etc/security/user 파일이 없습니다." >> $CREATE_FILE 2>&1
fi
...
echo " " > passwordpolicy.txt
  ...
  if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minlen" | egrep [0-9]| awk -F= '{print $2}'| wc -l` -eq 0 ]
    then
      echo "● 1.04 결과 : 취약" >> passwordpolicy.txt
    else
     if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minlen" | awk -F= '{print $2}'` -ge 8 ]
      then
  ...
  if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "maxage" | egrep  [0-9]| awk -F= '{print $2}'| wc -l ` -eq 0 ]
   then
     echo "● 1.04 결과 : 취약" >>passwordpolicy.txt
   else
    if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "maxage" | awk -F= '{print $2}'` -le 12 ]
     then
       echo "● 1.04 결과 : 양호" >> passwordpolicy.txt
     else
       echo "● 1.04 결과 : 취약" >> passwordpolicy.txt
   ...
   if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minage" |awk -F= '{print $2}'|wc -l` -eq 0 ]
    then
      echo "● 1.04 결과 : 취약" >> passwordpolicy.txt
    else
     if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "minage" |awk -F= '{print $2}'` -eq 1 ]
      then
       echo "● 1.04 결과 : 양호" >> passwordpolicy.txt
      else
       echo "● 1.04 결과 : 취약" >> passwordpolicy.txt
...
if [ -f /etc/security/passwd ]
  then
    echo "☞ /etc/security/passwd 파일 " >> $CREATE_FILE 2>&1
    cat /etc/security/passwd >> $CREATE_FILE 2>&1
  else
    echo "/etc/security/passwd 파일이 없습니다. " >> $CREATE_FILE 2>&1
fi

echo " " >> $CREATE_FILE 2>&1
echo "☞ Crack 파일 " >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
...
sed -e 's/^PTAG://'  >> `hostname`_passwd.txt
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : /etc/passwd의 두번째 필드가 'x'로 되어 있고, /etc/security/passwd 가 존재하면 양호">> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
if [ -f /etc/security/passwd ]
  then
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : /etc/passwd, /etc/group 파일의 권한이 444 또는 644이고 /etc/security/passwd 파일의 권한이 400 또는 600이면 양호" >> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
if [ -f /etc/passwd ]
  then
...
if [ -f /etc/security/passwd ]
  then
    ls -alL /etc/security/passwd >> $CREATE_FILE 2>&1
  else
    echo " /etc/security/passwd 파일이 없습니다." >> $CREATE_FILE 2>&1
fi

if [ -f /etc/security/group ]
  then
    ls -alL /etc/security/group >> $CREATE_FILE 2>&1
  else
    echo " /etc/security/group 파일이 없습니다." >> $CREATE_FILE 2>&1
fi
...
if [ -f /etc/security/passwd ]
  then
if [ `ls -alL /etc/security/passwd | grep ".r.-------" | wc -l` -eq 1 ]
  then
    echo "● 2.01 결과 : 양호" >> userfile.txt
  else
    echo "● 2.01 결과 : 취약" >> userfile.txt
...
if [ -f /etc/security/group ]
  then
if [ `ls -alL /etc/security/group | grep ".r.-------" | wc -l` -eq 1 ]
  then
    echo "● 2.01 결과 : 양호" >> userfile.txt
  else
    echo "● 2.01 결과 : 취약" >> userfile.txt
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : /etc/security/user 파일이 타사용자에게 쓰기권한이 없으면 양호" >> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
if [ -f /etc/security/user ]
  then
   ls -alL /etc/security/user  >> $CREATE_FILE 2>&1
  else
   echo " /etc/security/user 파일이 없습니다"  >> $CREATE_FILE 2>&1
fi
echo " " >> $CREATE_FILE 2>&1
...
if [ -f /etc/security/user ]
  then
    if [ `ls -alL /etc/security/user | awk '{print $1}' | grep '........-.' |wc -l` -eq 0 ]
       then
...
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
cro="/var/spool/cron/crontabs/* /var/adm/cron/cron.allow /var/adm/cron/cron.deny"
...
echo "■ 현황 " >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
FILES="/var/adm/wtmpx /var/adm/utmpx /var/adm/wtmp /var/adm/utmp /var/adm/sulog /var/adm/authlog /var/adm/messages /usr/adm/wtmpx /usr/adm/utmpx /usr/adm/wtmp /usr/adm/utmp /usr/adm/btmp /usr/adm/syslog /usr/adm/sulog /usr/adm/messages /usr/adm/lastlog /etc/security/lastlog /etc/security/failedlogin"
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : UMASK 값이 022 또는 027이면 양호(UMASK값 필수, /etc/profile, /etc/security/user 중에 하나만 되어 있어도 양호)" >> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo "  " >> $CREATE_FILE 2>&1
...
if [ -f /etc/security/user ]
    then
     echo "default 값" >> $CREATE_FILE 2>&1
     awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "umask" | grep "=" >> $CREATE_FILE 2>&1
    else
      echo "/etc/security/user 파일이 없습니다." >> $CREATE_FILE 2>&1
fi
...
if [ -f /etc/profile ]
  ...
  if [ -f /etc/security/user ]
     then
      if [ `awk '/default:/,/root:/' /etc/security/user | grep -v '^ *#'| grep -i "umask" | awk -F= '{print $2}'` -eq 022 ]
       then
         echo "● 3.01 결과 : 양호" >> mask.txt
        else
         echo "● 3.01 결과 : 취약" >> mask.txt
...
if [ `ls -al /usr/dt/bin/dtaction /usr/dt/bin/dtterm /usr/bin/X11/xlock /usr/sbin/mount /usr/sbin/lchangelv |egrep ".rws......|....rws..." |wc -l` -eq 0 ]; then
	echo "불필요하게 setuid, setgid가 설정된 파일이 존재하지 않습니다." >> $CREATE_FILE 2>&1
    echo "● 3.02 결과 : 양호" >> $CREATE_FILE 2>&1
  else
    ls -al /usr/dt/bin/dtaction /usr/dt/bin/dtterm /usr/bin/X11/xlock /usr/sbin/mount /usr/sbin/lchangelv |egrep ".rws......|....rws..." >> $CREATE_FILE 2>&1
...
if [ `cat telnetps.txt | grep telnetd | grep -v grep | wc -l` -gt 0 ]
 then
   ...
   if [ -f /etc/security/login.cfg ]
    then
     if [ `cat /etc/security/login.cfg | grep -v '#' | grep -v "*" | grep -i "herald" | grep "=" | wc -l` -eq 0 ]
      then
        echo "● 4.01 결과 : 취약" >> banner.txt
        echo "/etc/default/telnetd 파일 설정 없습니다." >> $CREATE_FILE 2>&1
      else
        echo "● 4.01 결과 : 양호" >> banner.txt
        echo "/etc/security/login.cfg 파일 내용" >> $CREATE_FILE 2>&1
        cat /etc/security/login.cfg | grep -v '#' | grep -v "*" | grep -i "herald" >> $CREATE_FILE 2>&1
     fi
    else
     echo "● 4.01 결과 : 취약" >> banner.txt
     echo "/etc/security/login.cfg 파일 존재하지 않습니다." >> $CREATE_FILE 2>&1
   fi
 else
  echo "● 4.01 결과 : 양호" >> banner.txt
...
if [ `cat ftpps.txt | grep ftpd | grep -v grep | wc -l` -gt 0 ]
 then
  ...
  if [ -f /usr/lib/nls/msg/$ftp_LANG1/ftpd.cat ]
    then
    if [ `cat /usr/lib/nls/msg/$ftp_LANG1/ftpd.cat | grep "%s FTP" | grep "(%s)" | wc -l` -gt 0 ]
     then
        echo "● 4.01 결과 : 취약" >> banner.txt
        cat /usr/lib/nls/msg/$ftp_LANG1/ftpd.cat | grep "%s FTP" | grep "(%s)" >> $CREATE_FILE 2>&1
      else
        echo "● 4.01 결과 : 양호" >> banner.txt
        cat /usr/lib/nls/msg/$ftp_LANG1/ftpd.cat | grep "%s Authorized" >> $CREATE_FILE 2>&1
     fi
    else
     echo "● 4.01 결과 : 수동" >> banner.txt
     echo "/usr/lib/nls/msg/$ftp_LANG1/ftpd.cat 파일 존재하지 않습니다. 다른 경로를 점검해야 합니다." >> $CREATE_FILE 2>&1
   fi
 else
  echo "● 4.01 결과 : 양호" >> banner.txt
...
if [ `ps -ef | grep sendmail | grep -v grep | wc -l` -gt 0 ]
 then
   ...
   if [ -f /etc/sendmail.cf ]
     then
       if [ `cat /etc/sendmail.cf | grep -i "GreetingMessage" | grep -i "Sendmail" | wc -l` -gt 0 ]
         then
           echo "● 4.01 결과 : 취약" >> banner.txt
           echo "/etc/sendmail.cf 파일 내용" >> $CREATE_FILE 2>&1
       ...
       echo "● 4.01 결과 : 수동" >> banner.txt
       echo "/etc/sendmail.cf 파일 존재하지 않습니다. 다른 경로를 점검해야 합니다." >> $CREATE_FILE 2>&1
   fi
 else
  echo "● 4.01 결과 : 양호" >> banner.txt
  ...
  then
    ...
    if [ -f /etc/named.conf ]
      then
       ...
       echo "● 4.01 결과 : 수동" >> banner.txt
       echo "/etc/named.conf 파일 존재하지 않습니다. 다른 경로를 점검해야 합니다." >> $CREATE_FILE 2>&1
   fi
 else
  echo "● 4.01 결과 : 양호" >> banner.txt
...
echo " " >> $CREATE_FILE 2>&1
ls -al /etc/rc.d/rc*.d/* | grep -i nfs | grep "/S" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
cat /etc/inittab | grep -i rcnfs >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
...
echo " " >> $CREATE_FILE 2>&1
ls -al /etc/rc.d/rc*.d/* | grep -i nfs | grep "/S" >> $CREATE_FILE 2>&1
ls -al /etc/rc.d/rc*.d/* | grep -i auto | grep "/S" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
...
echo " " >> $CREATE_FILE 2>&1
ls -al /etc/rc.d/rc*.d/* | grep -i sendmail | grep "/S" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
cat /etc/rc.tcpip | grep -i sendmail >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1

echo "② /etc/sendmail.cf 파일의 옵션 확인" >> $CREATE_FILE 2>&1
if [ -f /etc/sendmail.cf ]
  then
    grep -v '^ *#' /etc/sendmail.cf | grep PrivacyOptions >> $CREATE_FILE 2>&1
  else
    echo "/etc/sendmail.cf 파일 없습니다." >> $CREATE_FILE 2>&1
fi
...
if [ `ps -ef | grep sendmail | grep -v "grep" | wc -l` -eq 0 ]
  then
     ...
     if [ -f /etc/sendmail.cf ]
      then
      if [ `cat /etc/sendmail.cf | grep -i "O PrivacyOptions" | grep -i "noexpn" | grep -i "novrfy" |grep -v "#" |wc -l ` -eq 1 ]
       then
         echo "● 4.10 결과 : 양호" >> $CREATE_FILE 2>&1
       else
...
if [ `ps -ef | grep sendmail | grep -v "grep" | wc -l` -eq 0 ]
  then
    ...
    if [ -f /etc/sendmail.cf ]
     then
     if [ `cat /etc/sendmail.cf | grep -i "O PrivacyOptions" | grep -i "restrictqrun" | grep -v "#" |wc -l ` -eq 1 ]
       then
         echo "● 4.11 결과 : 양호" >> $CREATE_FILE 2>&1
       else
...
if [ `ps -ef | grep sendmail | grep -v "grep" | wc -l` -eq 0 ]
  then
     ...
     if [ -f /etc/sendmail.cf ]
      then
        if [ `cat /etc/sendmail.cf | grep -v "^#" | grep "R$\*" | grep -i "Relaying denied" | wc -l ` -gt 0 ]
          then
            echo "● 4.12 결과 : 양호" >> $CREATE_FILE 2>&1
          else
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : /etc/security/user에서 root: rlogin=false이면 양호" >> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1

if [ -f /etc/security/user ]
  then
    echo "root 값" >> $CREATE_FILE 2>&1
    awk '/root:/,/daemon:/' /etc/security/user | grep -v '^ *#'| grep -i "rlogin" | grep "=" >> $CREATE_FILE 2>&1
  else
    echo "/etc/security/user파일이 없습니다." >> $CREATE_FILE 2>&1
fi
...
if [ `cat telnetps.txt | grep telnetd | grep -v grep | wc -l` -gt 0 ]
  then
   if [ -f /etc/security/user ]
    then
     if [ `awk '/root:/,/daemon:/' /etc/security/user | grep -v '^ *#'| grep -i 'rlogin' | grep 'false' | wc -l` -eq 0 ]
      then
...
echo " " >> $CREATE_FILE 2>&1
ls -al /etc/rc.d/rc*.d/* | grep -i snmp | grep "/S" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1

echo "② /etc/snmpd.conf 파일 " >> $CREATE_FILE 2>&1
if [ -f /etc/snmpd.conf ]
        then
           grep -v '^ *#' /etc/snmpd.conf | egrep -i "public|private" | egrep -v "group|trap" >> $CREATE_FILE 2>&1
        else
          echo " /etc/snmpd.conf 파일이 존재하지 않음 " >> $CREATE_FILE 2>&1
fi
...
echo "--------------------------------------------------------------------------------------------------------" >> $CREATE_FILE 2>&1
echo "■ 기준 : AIX는 기본적으로 sulog가 설정되므로 양호" >> $CREATE_FILE 2>&1
echo "■ 현황" >> $CREATE_FILE 2>&1
echo " " >> $CREATE_FILE 2>&1
...
if [ `ps -ef | grep sendmail | grep -v "grep" | wc -l` -eq 0 ]
  then
    ...
    if [ -f /etc/sendmail.cf ]
     then
     if [ `grep -v '^ *#' /etc/sendmail.cf | egrep "DZ8.13.8|8.14.0|8.14.1" | wc -l ` -eq 1 ]
       then
         echo "● 6.01 결과 : 양호" >> $CREATE_FILE 2>&1
       else
